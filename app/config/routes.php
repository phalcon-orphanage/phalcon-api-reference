...
$router->add('/{language:[A-Za-z]{2,2}}/{version:((\d+\.\d+\.\d+)|latest)}/{class:(?i)phalcon(/[\w/]+)?}', array(
	'controller' => 'apireference',
	'action'     => 'showClass',
))->setName('showClass');

// this only need to get URL by route name
$router->add('/{language:[A-Za-z]{2,2}}/{version:((\d+\.\d+\.\d+)|latest)}/{type:(classes|namespaces|interfaces|changelog)}', array(
	'controller' => 'apireference',
	'action'     => 'index',
))->setName('showSummary');

$router->add('/?([A-Za-z]{2,2})?(/((\d+\.\d+\.\d+)|latest))?(/(classes|namespaces|interfaces|changelog))?', array(
	'controller' => 'apireference',
	'action'     => 'index',
	'language'   => 1,
	'version'    => 3,
...
$router->add('(/([A-Za-z]{2,2}))?(/((\d+\.\d+\.\d+)|latest))?/((?i)phalcon(/[\w/]+)?)', array(
	'controller' => 'apireference',
	'action'     => 'showClass',
	'language'   => 2,
	'version'    => 4,
